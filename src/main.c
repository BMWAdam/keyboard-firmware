...
#define LAYOUT_CONFIG_SIZE  (16 * 1024)
#define SETTINGS_SIZE       (4  * 1024)

#define FLASH_TARGET_OFFSET   (1024 * 1024)
#define FLASH_SETTINGS_OFFSET (FLASH_TARGET_OFFSET + LAYOUT_CONFIG_SIZE)

const uint8_t *flash_target_contents   = (const uint8_t *)(XIP_BASE + FLASH_TARGET_OFFSET);
const uint8_t *flash_settings_contents = (const uint8_t *)(XIP_BASE + FLASH_SETTINGS_OFFSET);
...
static char   json_buf[LAYOUT_CONFIG_SIZE];
...
void parse_json_to_ram() {
    ...
    memcpy(json_buf, flash_target_contents, LAYOUT_CONFIG_SIZE);
    json_buf[LAYOUT_CONFIG_SIZE - 1] = '\0';
...
void load_underglow_settings(void) {
    if (flash_settings_contents[0] == SETTINGS_MAGIC) {
        underglow_enabled = (flash_settings_contents[1] == 0x01);
    }
}

static void save_underglow_settings(void) {
    static uint8_t settings_page[FLASH_PAGE_SIZE];
    memset(settings_page, 0xFF, sizeof(settings_page));
    settings_page[0] = SETTINGS_MAGIC;
    settings_page[1] = underglow_enabled ? 0x01 : 0x00;

    uint32_t interrupts = save_and_disable_interrupts();
    flash_range_erase(FLASH_SETTINGS_OFFSET, FLASH_SECTOR_SIZE);
    flash_range_program(FLASH_SETTINGS_OFFSET, settings_page, FLASH_PAGE_SIZE);
...
void cdc_config_task(void *data) {
    ...
    static char     config_buffer[LAYOUT_CONFIG_SIZE];
    ...
    for (;;) {
        if (tud_cdc_available()) {
            ...
            if (strstr(rx_buffer, "CONFIG_BEGIN")) {
                ...
                {
                    uint32_t interrupts = save_and_disable_interrupts();
                    flash_range_erase(FLASH_TARGET_OFFSET, LAYOUT_CONFIG_SIZE);
            ...
            // ── Read Layout ──
                ...
                tud_cdc_write_flush();
                for (int i = 0; i < LAYOUT_CONFIG_SIZE; i += 64) {
                    char chunk[65];
            ...
            // ── Underglow toggle ──
                ...
                save_underglow_settings();
                tud_cdc_write_str(underglow_enabled ? "Underglow ON.\r\n" : "Underglow OFF.\r\n");
                tud_cdc_write_flush();
            }

            // ── Data capture ──
            else if (receiving_layout_cfg) {
                if (config_len + count < LAYOUT_CONFIG_SIZE) {
